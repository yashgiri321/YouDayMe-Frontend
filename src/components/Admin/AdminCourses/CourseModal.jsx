                ...
                <ModalBody p="16">
                    ...
                    <Grid templateColumns={['1fr','3fr 1fr']}>
                        <Box px={['0','16']}>
                         ...
                         {lectures.map((item,i)=>(
                         <VideoCard key={i} title="React Intro" 
                        ...
                        <Box>
                            <form onSubmit={e=>addLectureHandler(e,id,title,description,video)}>
                                <VStack spacing={"4"}>
                                    ...
                                    <Input focusBorderColor='purple.300' _placeholder="Title" value={title} onChange={e=> setTitle(e.target)}/>
                                    <Input focusBorderColor='purple.300' _placeholder="Description" value={description} onChange={e=> setDescription(e.target)}/>
                                   <Input accept='video/mp4' required type={'file'} focusBorderColor="purple.300" css={{'&::file-selector-button': {...fileUploadcss,color:'purple'},}} onChange={changeVideoHandler}/>
                                {videoPrev &&(
...
function VideoCard({title,descripton,num,lectureId,courseId,deleteButtonHandler}){
    return(
        <Stack direction={['column','row']} my="8" borderRadius={"lg"} boxShadow={"0 0 10px rgba(107,70,193,0.5)"} justifyContent={['flex-start','space-between']} p={['4','8']}>
            <Box>
                ...
                <Text children={descripton}/>
            </Box>
            <Button color={'purple.600'} onClick={()=>deleteButtonHandler(courseId,lectureId)}><RiDeleteBin7Fill/></Button>
        </Stack>
    )
}